...
class PlanesAlimenticios extends React.Component {
        ...
        planesalimenticios
            ...
            .then(snapshot => {
                snapshot.forEach(doc => {
                    if(doc.data().terminado === false) {
                        if(doc.data().usuario === this.state.pusuarioactual || doc.data().entrenador === this.state.pusuarioactual) {
                            this.setState({
                                pid: doc.id,
                            ...
                            li6.id = "dias";
                            li7.id = "fcha";
                            finalizar.className = "botones";
                            finalizar.id = "fnalizar";
                            ...
                            finalizar.textContent = "¿Finalizar rutina?";
                            finalizar.onclick = this.terminar;
    ...
    terminar() {
        planesalimenticios.doc(this.state.pid).update({
            terminado: true
        }).then(
            setTimeout(function () {
                window.location.href = "/planes-alimenticios";
            }, 500))
    }
        ...
        return (
            <div>
                ...
                <div className="contenido">
                    <p className="titulo">TUS PLANES ALIMENTICIOS</p>
                    <br />
                    <ul id="atributos"></ul>
                </div>
            </div>
...
export default withRouter(PlanesAlimenticios);